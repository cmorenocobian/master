...
for local,i in tqdm(enumerate(d)):
    ...
    if same>1 and len(dist)<400:
        eleccion.append([local, same, dist])
    
del dist, i, same, tipo, x, y
...
# local = local.loc[(local['price'] < 5000) & (local['size'] > 200) & (local['size'] < 600)]

# De estos locales podemos analizar la competencia
lista =[]
for i in eleccion:
    if i[0] in  local.index:
        x= pd.DataFrame(rest.loc[i[2], :][['media', 'rating', 'review_count','Poblacion', 
